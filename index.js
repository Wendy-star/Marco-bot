...
const HandleMsg = require('./HandleMsg')
   ...
   const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('AGHX BOT', { horizontalLayout: 'fullt' })))
    console.log(color(figlet.textSync('<<---------->>', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Aghx', 'white'))
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `اهلا وسهلا بك في مجموعتنا اللطيفه  @${event.who.replace('@c.us', '')}املين لك كل التوفيق والمحبه🎈🤍`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `نراك لاحقا 👋 @${event.who.replace('@c.us', '')},`)
        }
    })
    // })
